...
{
    public class BleDevice : DeviceBase<BluetoothDevice>
    {
        /// <summary>
        ...
        public BleDevice(BleAdapter adapter, BluetoothDevice nativeDevice, BluetoothGatt gatt, int rssi = 0, byte[] advertisementData = null, bool isConnectable = true)
        ...
        protected override async Task<IReadOnlyList<IGattService>> GetServicesNativeAsync()
        {
            ...
            {
                return _gatt.Services.Select(service => new GattService(service, _gatt, _gattCallback, this)).ToList();
            }

            return await DiscoverServicesInternal();
        }

        protected override async Task<IGattService> GetServiceNativeAsync(Guid id)
        {
            if (_gattCallback == null || _gatt == null)
            {
                return null;
            ...
            {
                return new GattService(nativeService, _gatt, _gattCallback, this);
            }
        ...
        private async Task<IReadOnlyList<IGattService>> DiscoverServicesInternal()
        {
            if (_gatt == null)
            {
            ...
            return await TaskBuilder
                .FromEvent<IReadOnlyList<IGattService>, EventHandler<ServicesDiscoveredCallbackEventArgs>, EventHandler>(
                    execute: () =>
                    {
                    ...
                    getCompleteHandler: (complete, reject) => ((sender, args) =>
                    {
                        ...
                        {
                            complete(new List<IGattService>());
                        }
                        else
                        {
                            complete(_gatt.Services.Select(service => new GattService(service, _gatt, _gattCallback, this)).ToList());
                        }
                    }),
