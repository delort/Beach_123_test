...
namespace InterviewBle.iOS.Services
{
    public class Device : DeviceBase<CBPeripheral>
    {
        ...
        protected override Task<IReadOnlyList<IGattService>> GetServicesNativeAsync()
        {
        ...
        private Task<IReadOnlyList<IGattService>> GetServicesInternal(CBUUID id = null)
        {
            var exception = new Exception($"Device {Name} disconnected while fetching services.");

            return TaskBuilder.FromEvent<IReadOnlyList<IGattService>, EventHandler<NSErrorEventArgs>, EventHandler<CBPeripheralErrorEventArgs>>(
                    execute: () =>
                    {
